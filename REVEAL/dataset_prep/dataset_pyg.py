...
class PygNodePropPredDataset(InMemoryDataset):
    def __init__(self, name, num_class = 0, root = root_folder+'/dataset', transform=None, pre_transform=None, meta_dict = None):
        ...
        self.download_name = self.meta_info['download_name'] ## name of downloaded file, e.g., tox2
    ...
    def process(self):
        ...
        if self.is_hetero:
            ...
            if 'classification' in self.task_type:
                ...
                data.y = torch.from_numpy(node_label).to(torch.float32)
            if self.binary:
                graph_feature = np.load(osp.join(self.raw_dir, 'graph-feature.npz'))['graph_feature']
            else:
                graph_feature = pd.read_csv(osp.join(self.raw_dir, 'graph-feature.csv.gz'), compression='gzip', header = None).values
            if np.isnan(node_label).any():
                data.graph_feature = torch.from_numpy(graph_feature).to(torch.float32)
            else:
                data.graph_feature= torch.from_numpy(graph_feature).to(torch.long)
