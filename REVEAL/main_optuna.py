...
from model import SynthNet,SimpleGNN1,SimpleGNN_SAGPool,BiDirectionalGraphSAGE
...
def run_study(args, train_dl_msb, test_dl_msb, valid_dl_msb, DUMP_DIR):
    ...
    study = optuna.create_study(
        direction='maximize',  # Correct direction for maximizing accuracy
...
# def objective(trial, args, train_dl_msb, valid_dl_msb, test_dl_msb, DUMP_DIR):
#     global best_metrics
...
#     args.num_gnn_layers  = num_layers
...
#     optimizer_msb = torch.optim.Adam(model_msb.parameters(), lr=learning_rate, weight_decay=weight_decay)
...
#         # Validation
#         valid_loss_msb_value, accuracy, top2_accuracy, top3_accuracy = evaluate(model_msb, device, valid_dl_msb, task=2)
...
#         # if valid_loss_msb_value < best_val_loss:
#         #     best_val_loss = valid_loss_msb_value
#         #     best_model_state = model_msb.state_dict()

#         # Update best test metrics
#         if accuracy1 > best_test_metrics[0]:
#             best_test_metrics = (accuracy1, top2_accuracy1, top3_accuracy1)
#             best_model_state = model_msb.state_dict()
#             best_val_loss = valid_loss_msb_value
#     # Set user attributes for the trial
#     trial.set_user_attr(key="best_model_state", value=best_model_state)
#     trial.set_user_attr(key="best_test_metrics", value=best_test_metrics)

#     return best_val_loss
def objective(trial, args, train_dl_msb, test_dl_msb, valid_dl_msb, DUMP_DIR, tracker=None):
    # Remove global declarations as they are unnecessary
    ...
    trial_params = {
        ...
        'num_layers': trial.suggest_int('num_layers', 2, 6),
        'gnn_embedding_dim': trial.suggest_int('gnn_embedding_dim', 32, 256),
        'hidden_channels': trial.suggest_int('hidden_channels', 64, 512),
        'dropout': trial.suggest_uniform('dropout', 0.1, 0.7),
        'weight_decay': trial.suggest_loguniform('weight_decay', 1e-6, 1e-3),
        'batch_size': trial.suggest_categorical('batch_size', [256,512, 1024]),
        'pooling_ratio': trial.suggest_uniform('pooling_ratio', 0.3, 0.7),
        'num_epochs': trial.suggest_int('num_epochs', 100, 400),  # Added parameter
    }
    ...
    optimizer_msb = torch.optim.Adam(model_msb.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        # Report intermediate objective value (validation accuracy)
        ...
        if tracker.update_best_top1(best_test_metrics, model_msb.state_dict(), trial_params):
            ...
            # Save best Top-1 model and related information
            model_path = os.path.join(DUMP_DIR, 'best_top1_model.pt')
        ...
        if tracker.update_best_top3(best_test_metrics, model_msb.state_dict(), trial_params):
            ...
            # Save best Top-3 model and related information
            model_path = os.path.join(DUMP_DIR, 'best_top3_model.pt')
    ...
    # Return the best validation accuracy for maximization
...
def main():
    ...
    parser.add_argument('--n_trials', type=int, default=200, help='Number of Optuna trials')
    ...
    if args.mode == 'train':
        ...
        args.num_gnn_layers  = best_params.get('num_layers', args.num_layers)
        ...
        args.weight_decay = best_params.get('weight_decay', args.weight_decay)
        args.batch_size = best_params.get('batch_size', args.batch_size)
        args.pooling_ratio= best_params.get('pooling_ratio',args.pooling_ratio)
